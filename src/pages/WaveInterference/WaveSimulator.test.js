...
test('WaveSimulator: initialization', () => {
    ...
    expect(sim.height).toBe(10);
    expect(sim.size).toBe(100);
    expect(sim.buffer1.length).toBe(100);
    expect(sim.buffer2.length).toBe(100);
    expect(sim.walls.length).toBe(100);
...
test('WaveSimulator: reset', () => {
    ...
    expect(sim.sources.length).toBe(0);
    expect(sim.walls[2 * 10 + 2]).toBe(0);
    expect(sim.time).toBe(0);
});
...
test('WaveSimulator: setWall', () => {
    ...
    sim.setWall(2, 2, true);
    // index = 2 * 5 + 2 = 12
    expect(sim.walls[12]).toBe(1);

    sim.setWall(2, 2, false);
    expect(sim.walls[12]).toBe(0);
});
...
test('WaveSimulator: addDisturbance', () => {
    ...
    expect(sim.current[2 * w + 2]).toBe(1.0);
    ...
    for (const [x, y] of boundaries) {
        ...
        expect(sim.current[y * w + x]).toBe(0, `Disturbance should not be allowed at boundary (${x}, ${y})`);
    }
    ...
    for (const [x, y] of outOfBounds) {
        ...
        expect(sim.current.every(val => val === 0), `Out of bounds disturbance at (${x}, ${y}) should have no effect`).toBeTruthy();
    }
});
...
test('WaveSimulator: addSource', () => {
    ...
    expect(sim.sources[0]).toEqual({ x: 5, y: 5, frequency: 0.5, amplitude: 100, phase: 0 });
});
...
test('WaveSimulator: step (propagation)', () => {
    ...
    expect(Math.abs(sim.previous[neighborIdx] - 0.99) < 0.001, `Expected ~0.99, got ${sim.previous[neighborIdx]}`).toBeTruthy();
});
...
test('WaveSimulator: renderToBuffer', () => {
    ...
    // G = 40 * 0.2 = 8
    // B = 40 * 0.6 = 24
    expect(data[8]).toBe(32);
    expect(data[9]).toBe(8);
    expect(data[10]).toBe(24);
